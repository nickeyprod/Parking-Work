...
extension Level1 {
    func setupCars() {
        ...
        oldCopper = Car(scene: self, name: CarNameList.OldCopper.rawValue)
        chowerler = Car(scene: self, name: CarNameList.Chowerler.rawValue)
        ...
        chowerler!.node?.physicsBody = SKPhysicsBody(texture: SKTexture(imageNamed: CAR_TEXTURE_NAMES.chowerler), size: CGSize(width: 220, height: 440))
