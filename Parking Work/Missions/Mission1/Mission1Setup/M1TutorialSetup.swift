...
extension Mission1 {
    ...
    func createTutorial() {
        ...
        let tutorialWindow = SKSpriteNode(color: .black, size: CGSize(width: displayWidth! / 2, height: displayHeight! / 2))
        tutorialWindow.alpha = 0.8
        tutorialWindow.zPosition = 20
        ...
        animateTutorialButton(button: nextBtn)
        
        self.tutorialWindow = tutorialWindow
        
        adjustTutorialWindowHeight(label: messageLabel, nextBtn: nextBtn)
    }
    
    func adjustTutorialWindowHeight(label: SKLabelNode, nextBtn: SKShapeNode) {
        tutorialWindow?.size = CGSize(width: label.frame.width + 40, height: label.frame.height + nextBtn.frame.height + 35)
        tutorialWindow?.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        nextBtn.position = CGPoint(x: tutorialWindow!.frame.maxX - (nextBtn.frame.width / 2) - 20, y: tutorialWindow!.frame.minY + (nextBtn.frame.height) )
        label.verticalAlignmentMode = .top
        label.position = CGPoint(x: 0, y: tutorialWindow!.frame.maxY - 15)
    }
    ...
    func nextTutorialMsg() {
        ...
        if tutorialMsg == 1 {
            self.cameraNode?.run(moveAct, completion: {
                ...
                self.adjustTutorialWindowHeight(label: messageLabel!, nextBtn: nextBtn!)
            })
        }
            ...
            Timer.scheduledTimer(withTimeInterval: 0.4, repeats: false) { _ in
                self.cameraNode?.run(SKAction.scale(to: self.maxScale, duration: 1.0))
